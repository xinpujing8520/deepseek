...
title: "DeepSeek 处理长文本：分段比一次塞进去稳"
description: "长文处理场景下的 DeepSeek 使用记录，包含 2026-06-16 的实测过程和可复用做法。"
date: 2026-06-16
category: "长文处理"
tags:
  - deepseek
...
这类「长文处理」任务，我后来会加一个“检查模式”。第一轮只让 DeepSeek 给结果，第二轮让它检查有没有漏掉条件，第三轮再让它把结果压短。三轮看起来多，其实每轮都很短，比一口气要求它写完更稳。
...
我会把「长文处理」拆成四个动作：收集材料、清理无关内容、生成第一版、人工复查。DeepSeek 主要参与中间两步。它不负责决定最终方向，也不负责替你确认事实。

这个分工很重要。把所有事情都交给它，结果容易变得像一篇漂亮但空的说明；只让它做整理和表达，反而更像一个能用的工具。2026-06-16 这次测试里，我明显感觉到，任务越小，反馈越快。
